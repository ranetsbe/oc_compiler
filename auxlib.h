//$Id: auxlib.h,v 1.3 2013/11/27 15:58:42 ranetsbe Exp ranetsbe $
...
const char* get_execname (void);
   //
   // Returns a read-only value previously stored by set_execname.
   //
...
void errprint_usage (void);
   //
   // Print program usage and set exitstatus to EXIT_FAILURE.
   //
...
#define DEBUGSTMT(FLAG,STMTS) \
        if (is_debugflag (FLAG)) { DEBUGF (FLAG, ""); STMTS eprintf("\n"); }
#define DEBUGX(FLAG,STMTS) \
        if (is_debugflag (FLAG)) { STMTS }
#endif
